...
import (
	...
	"github.com/paulsmith/computeruser/internal/herbie/providers/anthropic"
	"github.com/paulsmith/computeruser/internal/herbie/providers/openai"
)

func fromEnv() (provider.Provider, string, error) {
	name := strings.ToLower(strings.TrimSpace(os.Getenv("COMPUTER_USE_PROVIDER")))
	model := strings.TrimSpace(os.Getenv("COMPUTER_USE_MODEL"))
	if name == "" {
		return nil, "", errors.New("COMPUTER_USE_PROVIDER is required to split a compound goal")
	}
	if model == "" {
		return nil, "", errors.New("COMPUTER_USE_MODEL is required to split a compound goal")
	}
	...
	default:
		return nil, "", fmt.Errorf("unsupported COMPUTER_USE_PROVIDER %q (use openai, anthropic, or openrouter)", name)
	}
